...
class TransactionProvider with ChangeNotifier {
  ...
  Future<TransactionModel?> addTransaction({
    ...
    await FirebaseFirestore.instance
      ...
      this._transaction = TransactionModel.fromJson(data.data()!);
    ...
    await this._patientProvider!.addPatient(doctorId, dataPatient);
  ...
  confirmPayment(
      String? docId, String? doctorId, String imgUrl, String? userUid) async {
    _isLoading = true;

    await FirebaseFirestore.instance
        .doc('users/$userUid/transaction/$docId')
        .update({
    ...
    await FirebaseFirestore.instance
        .doc('doctor/$doctorId/transaction/$docId')
        .update({
      'payment_proof': imgUrl,
      'status': "Waiting for Confirmation",
    });
    ...
    await FirebaseFirestore.instance
        .doc('doctor/$doctorId/queue/$docId')
        .update({
      'payment_proof': imgUrl,
      'status': "Waiting for Confirmation",
    });
  ...
  updateStatus(
      String? docId, String? userId, String? status, String? doctorId) async {
    _isLoading = true;

    await FirebaseFirestore.instance
        .doc('users/$userId/transaction/$docId')
        .update({
      'status': status,
    });
